...
// Токен
  const TOKEN = '5914939593:AAE_Z0z8szZaHwcldQ6Nc3ZzOdwVIGxGXUE';

  // ID чата
  const CHATID = '-807264937';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name']) && !empty($_POST['phone'] && !empty($_POST['name2']) && !empty($_POST['name3']) && !empty($_POST['name4']) && !empty($_POST['name6']) )) {
    ...
    if (isset($_POST['name']) && !empty($_POST['name'])) {
        $txt .= "Дата и время: " . strip_tags(trim(urlencode($_POST['name']))) . "%0A";
    }
    if (isset($_POST['name2']) && !empty($_POST['name2'])) {
      $txt .= "Адрес: " . strip_tags(trim(urlencode($_POST['name2']))) . "%0A";
  }
  if (isset($_POST['name3']) && !empty($_POST['name3'])) {
    $txt .= "Количество рабочих: " . strip_tags(trim(urlencode($_POST['name3']))) . "%0A";
}
if (isset($_POST['name4']) && !empty($_POST['name4'])) {
  $txt .= "Условия работы: " . strip_tags(trim(urlencode($_POST['name4']))) . "%0A";
}

if (isset($_POST['name6']) && !empty($_POST['name6'])) {
  $txt .= "Оплата: " . strip_tags(trim(urlencode($_POST['name6']))) . "%0A";
}
if (isset($_POST['name7']) && !empty($_POST['name7'])) {
  $txt .= "Требования к рабочим: " . strip_tags(trim(urlencode($_POST['name7']))) . "%0A";
}

    // Номер телефона
    if (isset($_POST['phone']) && !empty($_POST['phone'])) {
        $txt .= "Контакты заказчика: " . strip_tags(trim(urlencode($_POST['phone']))) . "%0A";
    }
